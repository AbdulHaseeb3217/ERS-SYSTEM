...
export default function BikeRiderSupportChatScreen({ navigation, route }) {
  const [rider, setRider] = useState(route?.params?.rider || null);
  const [riderId, setRiderId] = useState(route?.params?.riderId || null);
  ...
  const loadRiderSession = async () => {
    ...
    try {
      ...
      if (riderDataRaw) {
        savedRider = JSON.parse(riderDataRaw);
      } else if (sessionRaw) {
      ...
      const finalRider = rider || savedRider;
      ...
      if (!riderId && finalRiderId) {
        setRiderId(finalRiderId);
      }
      ...
      console.log("Rider session parse error:", error.message);
  ...
  const loadChats = async () => {
    try {
      ...
      const session = await loadRiderSession();

      if (!session.id) return;

      const res = await fetch(
        buildUrl(`/api/support-chats/user/bike_rider/${session.id}`)
      );
      ...
      console.log("Load bike rider support chats error:", error.message);
      setChats([]);
    } finally {
      setLoading(false);
    }
  ...
  const createTicket = async () => {
    ...
    try {
      ...
      if (!session.id) {
        Alert.alert("Session Error", "Rider ID missing. Please login again.");
        return;
      }
      ...
      const res = await fetch(buildUrl("/api/support-chats/create"), {
        ...
        body: JSON.stringify({
          ...
          userType: "bike_rider",
          userName:
            session.data?.fullName || session.data?.name || "Bike Rider",
      ...
      <ScrollView contentContainerStyle={styles.container}>
        <View style={styles.createBox}>
          ...
          <TextInput
            ...
            placeholder="Subject e.g. Ride issue"
